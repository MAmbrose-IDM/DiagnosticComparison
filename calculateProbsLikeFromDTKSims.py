...
def calculate_prob_positive_sample(filename_suffix, all_sampling_dates, all_LHs, test_names):
    """
    Based on the probability distribution for the number of individuals in the population who would test positive if
        sampled (from the DTK simulation output and from false positive rates), calculate the probability that at
        least one sample will test positive given circulation (or given no circulation) if ss sample are taken.
        Store the results in a .p pickle file.
    :param filename_suffix: identifies the set of simulations used
    :param all_sampling_dates: list of days sampling occurred (different outputs calculated for each scenario)
    :param all_LHs: list of larval habitat multipliers used for the simulations
         (different outputs calculated for each scenario)
    :param test_names: names of the diagnostic tests used during sampling
    """
    ...
    for s1 in range(len(all_sampling_dates)):
        for s2 in range(len(all_LHs)):
            ...
            with open("simOutputs_DTK/prob_pos_sample_given_no_circulation_samplingDate%i_xLH%i_%s.p" % (all_sampling_dates[s1],
                                                                                                      round(all_LHs[s2] * 100),
                                                                                                      filename_suffix), "wb") as f:
...
def calculate_likelihood_given_observation(filename_suffix, all_sampling_dates, all_LHs, test_names,
                                           p_circulation, ss_values):
    """
    Based on the probability distribution for the number of individuals in the population who would test positive if
        sampled (from the DTK simulation output and from false positive rates), calculate the probability that at
        least one sample will test positive given circulation (or given no circulation) if ss sample are taken.
    ...
    :param all_LHs: list of larval habitat multipliers used for the simulations
         (different outputs calculated for each scenario)
    :param test_names: names of the diagnostic tests used during sampling
    :param p_circulation: list of the probability of circulation values to consider
    :param ss_values: list of the number of samples collected to consider
    """
    # To greatly reduce the difficulty of this problem, we assume the population size is constant at the mean value across
    #     all DTK simulations. This assumption may possibly give rise to some biases and will ideally be explored through
    #     a sensitivity analysis in the future (something as easy as using the max and min values and seeing whether
    ...
    # Likelihood of circulation and of no circulation given number of negative samples observed
    #   (repeat for each scenario)
    for s1 in range(len(all_sampling_dates)):
        for s2 in range(len(all_LHs)):
            with open("simOutputs_DTK/prob_num_pos_circulation_samplingDate%i_xLH%i_%s.p" % (all_sampling_dates[s1],
